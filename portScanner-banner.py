...
def main():
    ...
    hostinput = input("Enter the remote address or range to scan\n(Example of format is 192.168.0.1-255): ")
    scanhost = ip_manipulation(hostinput)
    ...
    for n in range(len(scanhost)):
        ...
        for port in range(sport, (eport + 1)):
            try:
                ...
                # connect_ex() used instead of connect() to get connection status information.
                if s.connect_ex((scanhost[n], port)) == 0: # Using connection status
                    banner = s.recv(2048)
    ...
    print("\nPort scan completed of", hostinput, "through TCP ports", sport,"-",eport)
